...
import {type UpdateValue, type Transaction, mutators} from '@rocicorp/zero';
import {
...
export function createMutators(authData: AuthData | undefined) {
  return mutators({
    ...
    async deleteIssue(tx: Transaction<Schema>, id: string) {
    ...
    async addEmojiToIssue(tx: Transaction<Schema>, args: AddEmojiArgs) {
      await addEmoji(authData, tx, 'issue', args);
    },

    async addEmojiToComment(tx: Transaction<Schema>, args: AddEmojiArgs) {
      await addEmoji(authData, tx, 'comment', args);
    },

    async removeEmoji(tx: Transaction<Schema>, id: string) {
    ...
    async removeComment(tx: Transaction<Schema>, id: string) {
      await assertIsCreatorOrAdmin(authData, tx.query.comment, id);
      await tx.mutate.comment.delete({id});
    },

    async createLabel(tx, {id, name}: {id: string; name: string}) {
    ...
    async setUserPref(tx, {key, value}: {key: string; value: string}) {
      assertIsLoggedIn(authData);
      const userID = authData.sub;
